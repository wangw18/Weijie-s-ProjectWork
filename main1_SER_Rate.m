clear;
close all;
Nt = 8; % number of trans antennas
K = 8;  % number of users
Rs = 1; % symbol rate
sigma2 = 1.8; % squre od noise density
d = 1; % distance between BS and Ues
Gt = 10; % Gain of Transmitter
pathloss=148.1+37.6*log10(d) + sqrt(10)*randn(1) - Gt; 
var=10^(-pathloss/10);
...
hold on;
title(['SER vs SNR with K = ', num2str(K), ', Nt = ',num2str(Nt)],'FontName','Times New Roman');
...
hold on;
title(['Sum Rate vs SNR with K = ', num2str(K), ', Nt = ',num2str(Nt)],'FontName','Times New Roman');